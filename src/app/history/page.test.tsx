...
describe("Workout history", () => {
  ...
  it("turns completed workouts into glanceable stats, consistency, and linked daily activity", async () => {
    ...
    expect(
      screen.getByRole("status", { name: "Loading workout history" }),
    ).toBeTruthy();
    await waitFor(() =>
    ...
    expect(
      screen.getByText("Current streak").parentElement?.textContent,
    ).toContain("2days");
    expect(
      screen.getByText("Time trained").parentElement?.textContent,
    ).toContain("45:00total");
    expect(screen.getByRole("heading", { name: "Today" })).toBeTruthy();
    expect(screen.getByRole("heading", { name: "Yesterday" })).toBeTruthy();
    expect(
      screen
        .getByRole("link", { name: /Full Body Strength/i })
        .getAttribute("href"),
    ).toBe("/videos/video-1");
    expect(
      screen.getByRole("link", { name: /Mobility Flow/i }).getAttribute("href"),
    ).toBe("/videos/video-2");
  ...
  it("offers a direct path to the video deck when no workouts exist", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByRole("heading", { name: "No workouts logged yet" }),
      ).toBeTruthy(),
    );
    expect(
      screen
        .getByRole("link", { name: "Choose a workout" })
        .getAttribute("href"),
    ).toBe("/videos");
  });
});
